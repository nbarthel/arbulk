  ...
  var PIview = createDataLoader(ShipmentViewDataComponent,{
      queries:[{
       ...
       var base = 'TShipmentents';
        ...
        this.url = PIview._buildUrl(base, {
                    "include" : ["TLocation" , "TCompany" ,{"relation" :"TShipmentDomestic","scope":{"include":["TShipmentType"]}},{"relation" :"TShipmentInternational","scope":{"include":["TSteamshipLine"]}},{"relation" : "TShipmentLots" ,"scope":{"include":["TPackagingInstructionLots"]}}]
       ...
       {    
          ...
          _.map(view.TShipmentLots,(data,index)=>{
                  if(view.isDomestic == 0){
                    var freightForwarder = view.TShipmentInternational[0].freightForwarder
                    var erd = moment(view.TShipmentInternational[0].earliestReturnDate).format("YYYY-MM-DD")
                    var cutOff = moment(view.TShipmentInternational[0].cargoCutoffDate).format("YYYY-MM-DD")
                    var vessel = view.TShipmentInternational[0].steamshipVessel
                    var puLocation = view.TShipmentInternational[0].containerPickupLocation
                    var returnLocation = view.TShipmentInternational[0].containerReturnLocation
                    var docCutoff = moment(view.TShipmentInternational[0].docCutoffDate).format("YYYY-MM-DD")
                    var steamShipline = view.TShipmentInternational[0].TSteamshipLine.name
                    }
           return(
                   ...
                   <td style ={{display : this.props.showBooking}}>{view.isDomestic == 1 ? view.TShipmentDomestic[0].bookingNumber : view.TShipmentInternational[0].bookingNumber}</td>
                   <td style ={{display : this.props.showPO}}>{'N/A'}</td>
                   <td style ={{display : this.props.showLot}}>{'N/A'}</td>
                   ...
                   <td>
                   <label className="control control--checkbox">
                   <input type="checkbox"  onClick={(e) => this.checkclick(e,data)} id = {data.TPackagingInstructionLots.length != 0 ? data.TPackagingInstructionLots[0].id : ''} value = {view.id}  onChange={(e)=>{this.props.checkboxChange(e,view)}} /><div className="control__indicator"></div>
                   </label>
                   </td>
                   </tr>
                  )
