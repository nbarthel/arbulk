...
render() {
		let buttons = _.map(this.props.buttonDisplay,(button,index) => {
			return (<button type="button" onClick = {(e) => {this.props.onButtonRemove(index,button)}} id={index} key={index} className="btn btn-default">{button}<span aria-hidden="true">&times;</span></button>)
		...
		return (
                ...
                <button type="button" name="clearall" onClick={this.props.onRemove} className="btn btn-default">Clear All</button>
            </div>
		);
	}
}