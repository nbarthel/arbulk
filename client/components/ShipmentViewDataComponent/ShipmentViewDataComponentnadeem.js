...
render(){
      ...
      var eno = 'N/A'
      if(alloc == view.numberOfContainers){
          eno = "YES"
      }else{
        eno = "NO"
      }
       ...
       {    
          ...
          _.map(view.TShipmentLots,(data,index)=>{
           ...
           return(
                   ...
                   <td style ={{display : this.props.showConfmd}}>{status == "UNCONFIRMED" ? "NO" : "YES"}</td>
                   ...
                   <td style ={{display : this.props.showInInvt}}>{data.TPackagingInstructionLots ? data.TPackagingInstructionLots.inInventory : '0'}</td>
